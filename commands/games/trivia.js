...
class Game {
    constructor(message, args) { // Defining vars and running the game logic
        ...
        this.reactions = ['🇦', '🇧', '🇨', '🇩'];
        this.question = '';
        this.idList = fetch('https://opentdb.com/api_category.php')
            .then(response => response.json())
            .then(data => { this.idList = data.trivia_categories; });
    }
    init() {
    ...
    async get_data(dif, cat) {
        ...
        if (dif && cat) {
            ...
            for (const item of this.idList) {
                if (item.name.toLowerCase().replace(' ', '').replace(' ', '')
                    .replace(' ', '')
    ...
    awaitReactions() {
        this.question_message.awaitReactions((reaction, user) => user.id === this.message.author.id && ['🇦', '🇧', '🇨', '🇩'].includes(reaction.emoji.name),
    ...
    execute(message, args) {
        if (args[0]) {
            if (args[0].toLowerCase() === 'cat' || args[0].toLowerCase() === 'category' || args[0].toLowerCase() === 'categories') {
                let categories = '';
                for (const item of this.idList) {
                    categories = `${categories + item.name.replace(' ', '').replace(' ', '').replace(' ', '')
                        .replace(' ', '')}\n`;
                }
        ...
        const game = new Game(message, args);
        game.init();
    },
};
