...
const fetch = require("node-fetch"); // used for getting the trivia questions
const atob = require('atob') // to change base64 to text
fetch('https://opentdb.com/api_category.php')
.then(response => response.json())
.then(data => id_list = data.trivia_categories);// gets a list of possible categories
...
module.exports = {
    ...
    guildOnly: true,
    aliases: ['triv'],
    examples: 'All sub-commands are optionall, to view a list of categories, use .trivia categories',
    subcommands: '.trivia <difficulty>\n.trivia <difficulty> <Categories>\n.trivia categories\n',
	execute(message, args, client) {
        if (args[0]) {
            if (args[0].toLowerCase() == 'cat' || args[0].toLowerCase() == 'category' || args[0].toLowerCase() == 'categories') {
                let categories = ''
                for (let i in id_list) {
                    categories = categories + id_list[i].name.replace(' ', '').replace(' ', '').replace(' ', '').replace(' ', '')+ '\n'
                }
                const category_embed = new discord.MessageEmbed()
                .setColor('#0099ff')
                .setTitle('Question Categories')
                .setDescription(categories)
                .addField('Difficulties', 'Easy\nMedium\nHard\nAny')
                .setFooter("When entering in a category, make sure to not use any spaces")
                return message.channel.send(category_embed)
            }// to generate the list to send the requester
        }
        class Game {
