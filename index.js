...
client.on('guildCreate', guild => {
    ...
    client.users.cache.get('315399139783344128').send(`New guild joined: ${guild.name} (id: ${guild.id}). This guild has ${guild.memberCount} members!`);
    client.users.cache.get('359367096150261770').send(`New guild joined: ${guild.name} (id: ${guild.id}). This guild has ${guild.memberCount} members!`);
});
...
const console_read = require('console-read-write');
var status
async function main() { 
	status = await console_read.ask('What would you like the status to be?\n')
